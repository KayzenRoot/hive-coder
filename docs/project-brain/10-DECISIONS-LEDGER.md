...
**Proposal materialised under:** `HCODER-WO-0024` Context Lock Delta 001 (review finding M-20-06); corrected under Deltas 002 and 003

`HCODER-WO-0024` establishes the durable law a future distribution/update capability must be built on, and no such law existed before it. The proposal has seven parts: exactly one canonical product-version source (`apps/desktop/src-tauri/tauri.conf.json` → `version`) with `Cargo.toml` and `package.json` as exact mirrors enforced by a drift gate; a single **bounded SemVer 2.0.0 profile** (the SemVer 2.0.0 grammar restricted to version strings of at most 128 characters) enforced identically by the product TypeScript parser and the Python drift gate, with exact arbitrary-precision numeric comparison for both core and prerelease identifiers and never floating-point conversion; a release channel (`stable` | `beta` | `dev`) defined as a *shape* of version, with version and channel forming a single identity that fails closed when individually valid but mutually incompatible; a closed update-lifecycle state machine whose whole install path — `ready`, `installing` and `success` as authenticity-dependent states — requires an accepted authenticity/integrity proof against an admission allowlist that is empty in this slice, making the path unreachable by construction and refusing both a status snapshot and a recorded history entry that claims it; a persisted-event law requiring declared states, a reachable source state, a declared legal edge and an outcome that edge could actually have produced, with a persisted-outcome vocabulary deliberately narrower than the live transition vocabulary; closed, plain-own-data status snapshots with exact key sets at every level and no accessor ever executed by validation; and a Hive-owned, inert `UpdateService` boundary that exposes no check, download, install, restart, signing, publication or mutation member.
