...
**Proposal materialised under:** `HCODER-WO-0024` Context Lock Delta 001 (review finding M-20-06)

`HCODER-WO-0024` establishes the durable law a future distribution/update capability must be built on, and no such law existed before it. The proposal has six parts: exactly one canonical product-version source (`apps/desktop/src-tauri/tauri.conf.json` → `version`) with `Cargo.toml` and `package.json` as exact mirrors enforced by a drift gate; strict SemVer 2.0.0 acceptance with exact arbitrary-precision numeric prerelease precedence, never floating-point comparison; a release channel (`stable` | `beta` | `dev`) defined as a *shape* of version, with version and channel forming a single identity that fails closed when individually valid but mutually incompatible; a closed update-lifecycle state machine whose install-ready boundary (`ready`, and `installing` again as defence in depth) requires an accepted authenticity/integrity proof against an admission allowlist that is empty in this slice, making install-ready unreachable by construction; closed, bounded, reconstructed status snapshots with exact key sets at every level; and a Hive-owned, inert `UpdateService` boundary that exposes no check, download, install, restart, signing, publication or mutation member.
...
**Recorded deficit history.** The first Work Order implementation head `4710a47e2e099b03baa7fd3e5665bfbc882c4c8d` passed Governance `35224933911` and Desktop Shell `35224933743` yet carried four HIGH and two MEDIUM defects: the authenticity gate sat on the install call rather than the install-ready boundary, version and channel were validated independently rather than as one identity, status snapshots were neither closed nor reconstructed, numeric prerelease precedence used unsafe floating-point conversion, the channel contract carried a second looser version parser, and this ADR was referenced without existing. Independent review `5236275753` returned `CORRECTION_REQUIRED` (CRITICAL `0` / HIGH `4` / MEDIUM `2`). The corrections are carried in the same Work Order under Context Lock Delta 001; green gates on that head did not mean the claimed properties held.

**Promotion gate.** `DEC-028` remains PROPOSED until the corrected exact head passes Governance and Desktop Shell, executable tests prove every property above including the adversarial cases, an independent HEDS returns unresolved HIGH/CRITICAL `0/0`, and the Work Order merges with expected-head protection. No promotion claim may be made before then.
