...
**Status:** CANDIDATE  
`HCODER-WO-0011` proposes a measurable specialist layer above CP-0010. Versioned Expertise Capsules encode role doctrine, anti-patterns, review lenses and evaluation dimensions but grant no authority. Agent Profiles are trusted-host sealed and bind role/capsule/context/independence identity. ContextLens separates trusted context from untrusted data and fails closed when authoritative required context cannot fit the budget. Code Truth Map facts require provenance and host verification; Architectural Genome invariants bind to verified facts and the Project Digital Twin and expose drift. Experience Routing uses only trusted benchmark evidence with sample floors, multi-suite diversity and Wilson lower confidence bounds; model/provider names and self-description are absent from the scoring formula. AgentMesh assigns sealed MasterPlan steps only to measured specialists and separates oversight lineages from implementation. CounterPlan and Failure Oracle are host-identified adversarial challenge surfaces with no permission or promotion authority. Promotion requires exact-head Governance and HEDS.