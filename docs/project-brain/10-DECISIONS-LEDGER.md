...
**Status:** CANDIDATE  
`HCODER-WO-0012` proposes a repository-aware evaluation layer above CP-0011. RepoDNA creates bounded deterministic text snapshots without importing or executing repository code; secret-like files, symlinks and common binary artifacts are excluded by default and resource ceilings fail closed. TruthWeave derives reproducible Code Truth facts bound to exact snapshot/file/rule identity. GenomePulse mines evidence-bound repository-footprint invariants from verified facts plus explicit trusted Digital Twin node/path bindings rather than model inference. ShadowBench creates host-keyed hidden evaluation identities from current verified facts; its oracle remains represented by host-bound digests, while Benchmark Novelty rejects exact, semantic and source-lineage replay. Counterfactual Forge creates non-mutating what-if probes tied to the invariants expected to drift. Competence Half-Life and the Recertification Clock bind production certification to exact Agent Profile, Competence Standard and repository snapshot and can mark stale/changed evidence DUE or EXPIRED but cannot promote rank. Outcome Echo is trusted-host sealed and advisory-only: negative outcomes may accelerate recertification, positive outcomes cannot mint benchmark evidence. Language/framework Expertise Capsule extensions are descriptive and non-authoritative. Existing CP-0005/0007/0008/0009/0010/0011 authority boundaries remain unchanged. Promotion requires exact-head Governance and HEDS.
