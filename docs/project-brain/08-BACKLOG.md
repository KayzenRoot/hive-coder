...
17. `HCODER-WO-0017` Runtime Observability Contract & Safe Status Export — **DONE / CANONICAL `HCODER-CP-0017`**. Product PR #42 squash-merged as `00bcb87251772cba0eb385d9628448374e9dd612`; post-merge Governance #236 and Desktop Shell #72 SUCCESS; final product HEDS `5215281964`, unresolved HIGH/CRITICAL 0.
18. `HCODER-WO-0018` Cross-Runtime Status IPC Contract — **STAGED NEXT / RECONSTRUCTION REQUIRED**. Historical stacked implementation exists but must be replayed onto canonical CP-0017 and reconciled against the strict corrected `RuntimeStatusSnapshot v1` contract before promotion.
...
- **Runtime Observability:** strict bounded `RuntimeStatusSnapshot v1` is canonical through CP-0017. Presentation state is non-authoritative and uses canonical provenance plus fail-closed UNKNOWN/DISCONNECTED/DEGRADED semantics.
...
## Next NECESSARY product increment after canonical CP-0017
Reconstruct `HCODER-WO-0018 — Cross-Runtime Status IPC Contract` on canonical CP-0017. The reconstruction must consume the corrected strict `RuntimeStatusSnapshot v1` schema/provenance/fail-closed rules, preserve status as non-authoritative, and carry no divergent pre-squash ancestry from historical PR #37.
