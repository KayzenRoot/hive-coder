...
- CR-001 binds active global budgets into authenticated checkpoint state. Restart configuration must match; increases require explicit monotonic `extend_budget()` while paused and create a history event.
- Pause, continue, cancellation, terminal-state invariants and structured execution history exist.
- CR-002 preserves an in-flight scheduling pause when the current call settles; terminal non-retryable failure may still win.
...
## Promotion gate
Promote only after corrected exact-head Governance and HEDS approval with no unresolved HIGH/CRITICAL findings.
