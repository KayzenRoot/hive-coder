...
def _file_identity(info):return f"posix-file:{info.st_dev}:{info.st_ino}"
class PosixPreparedReplace:
    def __init__(self,backend,parent_fd,parent_parts,leaf,target_fd):
        self._backend,self._parent_fd,self._parent_parts,self._leaf,self._target_fd=backend,parent_fd,parent_parts,leaf,target_fd;self._closed=False;self._stage_fd=None;self._stage_name=None;self._stage_identity=None;self._stage_digest=None;self._stage_bytes=None;self._published=False
        ...
        digest,length=_digest_fd(target_fd);self.observed=WorkspaceReplaceObservedState(backend._dir_identity(parent_info),_file_identity(target_info),digest,length)
    ...
    def _revalidate_stage_path(self):
        if None in (self._stage_fd,self._stage_name,self._stage_identity,self._stage_digest,self._stage_bytes):raise WorkspaceBoundaryError("replacement staging is not fully verified")
        pinned_info=os.fstat(self._stage_fd)
        if not stat.S_ISREG(pinned_info.st_mode) or _file_identity(pinned_info)!=self._stage_identity:raise WorkspaceBoundaryError("pinned replacement staging identity changed")
        pinned_digest,pinned_length=_digest_fd(self._stage_fd)
        if pinned_digest!=self._stage_digest or pinned_length!=self._stage_bytes:raise WorkspaceBoundaryError("pinned replacement staging bytes changed")
        live_fd=None
        try:
            flags=os.O_RDONLY|getattr(os,"O_CLOEXEC",0)|getattr(os,"O_NOFOLLOW",0)
            try:live_fd=os.open(self._stage_name,flags,dir_fd=self._parent_fd)
            except OSError as exc:raise WorkspaceBoundaryError("replacement staging pathname cannot be reopened no-follow") from exc
            live_info=os.fstat(live_fd)
            if not stat.S_ISREG(live_info.st_mode) or _file_identity(live_info)!=self._stage_identity:raise WorkspaceBoundaryError("replacement staging pathname identity changed")
            digest,length=_digest_fd(live_fd)
            if digest!=self._stage_digest or length!=self._stage_bytes:raise WorkspaceBoundaryError("replacement staging pathname bytes changed")
        finally:
            if live_fd is not None:os.close(live_fd)
    def stage_replace(self,content,expected):
        ...
        self.revalidate_expected(expected);flags=os.O_RDWR|os.O_CREAT|os.O_EXCL|getattr(os,"O_CLOEXEC",0)|getattr(os,"O_NOFOLLOW",0)
        for _ in range(32):
            ...
            # Ownership is frozen immediately after O_EXCL creation, before writes.
            info=os.fstat(fd)
            ...
            self._stage_identity=_file_identity(info);break
        ...
        try:
            ...
            while offset<len(view):
                ...
                offset+=written
            os.fsync(self._stage_fd);digest,length=_digest_fd(self._stage_fd);wanted=hashlib.sha256(content).hexdigest()
            if digest!=wanted or length!=len(content):raise WorkspaceMutationError("replacement staging bytes failed verification")
            self._stage_digest,self._stage_bytes=digest,length;self._revalidate_stage_path()
        except Exception:self._cleanup_stage();raise
    def mutation_ready(self,expected):
        self._revalidate_stage_path();self.revalidate_expected(expected)
    def publish_replace(self,expected,pre_publish_check:Callable[[],None]):
        if None in (self._stage_fd,self._stage_name,self._stage_identity,self._stage_digest,self._stage_bytes):raise WorkspaceBoundaryError("replacement staging object is not mutation-ready")
        pre_publish_check();self.revalidate_expected(expected);self._revalidate_stage_path()
        ...
        self._published=True;self._stage_name=None;live_fd=None
        try:
            flags=os.O_RDONLY|getattr(os,"O_CLOEXEC",0)|getattr(os,"O_NOFOLLOW",0);live_fd=os.open(self._leaf,flags,dir_fd=self._parent_fd);info=os.fstat(live_fd)
            ...
            digest,length=_digest_fd(live_fd)
            if digest!=self._stage_digest or length!=self._stage_bytes:raise WorkspaceMutationError("published replacement bytes verification failed")
            return "replaced"
        finally:
            if live_fd is not None:os.close(live_fd)
    def _cleanup_stage(self):
        if self._stage_name is None:return
        # Never unlink a pathname whose identity was not captured and proven owned.
        if self._stage_identity is None:self._stage_name=None;return
        live_fd=None
        ...
        if live_identity!=self._stage_identity:self._stage_name=None;return
