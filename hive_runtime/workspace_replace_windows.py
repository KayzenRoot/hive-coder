...
def _write_handle_bytes(handle,content):
...
def _observed(parent_handle,target_handle):
...
class WindowsReplaceWorkspaceBackend(WindowsWorkspaceBackend):
    def prepare_replace(self,relative_path):
...
class WindowsPreparedReplace:
    ...
    def revalidate_expected(self,expected):
        ...
        if self._target_handle is None:raise WorkspaceBoundaryError("pinned Windows replacement target is no longer available before publication")
    ...
    def _open_publish_parent(self,expected):
        if not self._parent_parts:raise WorkspaceBoundaryError("root-level Windows replacement publication is not yet proven")
