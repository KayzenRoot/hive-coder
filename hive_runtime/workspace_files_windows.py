...
def _win_error(message: str) -> WorkspaceMutationError: return WorkspaceMutationError(f"{message} (winerror={ctypes.get_last_error()})")
...
def _nt_rename_relative(file_handle: int, parent_handle: int, leaf: str, *, replace_if_exists: bool) -> None:
    encoded = leaf.encode("utf-16-le"); offset = _FILE_RENAME_INFORMATION.FileName.offset; size = max(ctypes.sizeof(_FILE_RENAME_INFORMATION), offset + len(encoded)); raw = ctypes.create_string_buffer(size); info = _FILE_RENAME_INFORMATION.from_buffer(raw); info.u.ReplaceIfExists = 1 if replace_if_exists else 0; info.RootDirectory = wintypes.HANDLE(parent_handle); info.FileNameLength = len(encoded); ctypes.memmove(ctypes.addressof(raw) + offset, encoded, len(encoded)); iosb = _IO_STATUS_BLOCK(); status = int(_NtSetInformationFile(wintypes.HANDLE(file_handle), ctypes.byref(iosb), ctypes.byref(raw), size, _FILE_RENAME_INFORMATION_CLASS))
    if status < 0:
...
def _nt_rename_relative_no_clobber(file_handle: int, parent_handle: int, leaf: str) -> None: _nt_rename_relative(file_handle, parent_handle, leaf, replace_if_exists=False)
def _nt_rename_relative_replace(file_handle: int, parent_handle: int, leaf: str) -> None: _nt_rename_relative(file_handle, parent_handle, leaf, replace_if_exists=True)
...
class WindowsPreparedCreate:
    ...
    def publish(self, content: bytes, pre_publish_check: Callable[[], None]) -> str:
        ...
        try:
            ...
            pre_publish_check(); _nt_rename_relative_no_clobber(temp, self._parent_handle, self._leaf); published = True; final_info = _win_info(temp)
