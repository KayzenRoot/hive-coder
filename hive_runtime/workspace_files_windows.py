...
class _OBJECT_ATTRIBUTES(ctypes.Structure):
    _fields_ = [("Length", wintypes.ULONG), ("RootDirectory", wintypes.HANDLE), ("ObjectName", ctypes.POINTER(_UNICODE_STRING)), ("Attributes", wintypes.ULONG), ("SecurityDescriptor", wintypes.LPVOID), ("SecurityQualityOfService", wintypes.LPVOID)]
...
class _FILE_RENAME_INFORMATION(ctypes.Structure):
    _fields_ = [("u", _RENAME_UNION), ("RootDirectory", wintypes.HANDLE), ("FileNameLength", wintypes.ULONG), ("FileName", wintypes.WCHAR * 1)]
...
_GetFileInformationByHandle.argtypes = [wintypes.HANDLE, ctypes.POINTER(_BY_HANDLE_FILE_INFORMATION)]; _GetFileInformationByHandle.restype = wintypes.BOOL
_WriteFile = _kernel32.WriteFile
_WriteFile.argtypes = [wintypes.HANDLE, wintypes.LPCVOID, wintypes.DWORD, ctypes.POINTER(wintypes.DWORD), wintypes.LPVOID]; _WriteFile.restype = wintypes.BOOL
...
_GetFinalPathNameByHandleW.argtypes = [wintypes.HANDLE, wintypes.LPWSTR, wintypes.DWORD, wintypes.DWORD]; _GetFinalPathNameByHandleW.restype = wintypes.DWORD
_NtCreateFile = _ntdll.NtCreateFile
_NtCreateFile.argtypes = [ctypes.POINTER(wintypes.HANDLE), wintypes.DWORD, ctypes.POINTER(_OBJECT_ATTRIBUTES), ctypes.POINTER(_IO_STATUS_BLOCK), wintypes.LPVOID, wintypes.ULONG, wintypes.ULONG, wintypes.ULONG, wintypes.ULONG, wintypes.LPVOID, wintypes.ULONG]
_NtCreateFile.restype = _NTSTATUS
_NtSetInformationFile = _ntdll.NtSetInformationFile
_NtSetInformationFile.argtypes = [wintypes.HANDLE, ctypes.POINTER(_IO_STATUS_BLOCK), wintypes.LPVOID, wintypes.ULONG, wintypes.ULONG]
_NtSetInformationFile.restype = _NTSTATUS


class _NtOpenError(WorkspaceBoundaryError):
    def __init__(self, message: str, status: int) -> None:
        super().__init__(f"{message} (ntstatus=0x{status:08x})"); self.status = status
...
def _nt_open_relative(root_handle: int, name: str, *, desired_access: int, share_access: int, disposition: int, options: int, file_attributes: int = 0) -> int:
    buffer = ctypes.create_unicode_buffer(name); encoded_len = len(name.encode("utf-16-le")); string = _UNICODE_STRING(encoded_len, encoded_len + 2, ctypes.cast(buffer, wintypes.LPWSTR))
    attrs = _OBJECT_ATTRIBUTES(ctypes.sizeof(_OBJECT_ATTRIBUTES), wintypes.HANDLE(root_handle), ctypes.pointer(string), _OBJ_CASE_INSENSITIVE, None, None); iosb = _IO_STATUS_BLOCK(); out = wintypes.HANDLE()
    status = int(_NtCreateFile(ctypes.byref(out), desired_access, ctypes.byref(attrs), ctypes.byref(iosb), None, file_attributes, share_access, disposition, options, None, 0))
...
def _win_file_identity(info: _BY_HANDLE_FILE_INFORMATION) -> str:
    file_id = (int(info.nFileIndexHigh) << 32) | int(info.nFileIndexLow); return f"win-id:{int(info.dwVolumeSerialNumber)}:{file_id}"


def _win_file_state(info: _BY_HANDLE_FILE_INFORMATION) -> str:
    size = (int(info.nFileSizeHigh) << 32) | int(info.nFileSizeLow); mtime = (int(info.ftLastWriteTime.dwHighDateTime) << 32) | int(info.ftLastWriteTime.dwLowDateTime); return f"{_win_file_identity(info)}:{size}:{mtime}"
...
def _nt_rename_relative(file_handle: int, parent_handle: int, leaf: str, *, replace_if_exists: bool) -> None:
    """Atomically rename one already-open file into parent/leaf.

    This is a name-based NT publication primitive. replace_if_exists=True is an
    atomic namespace replacement, not an expected-file-id CAS predicate.
    """
    encoded = leaf.encode("utf-16-le"); offset = _FILE_RENAME_INFORMATION.FileName.offset; size = max(ctypes.sizeof(_FILE_RENAME_INFORMATION), offset + len(encoded)); raw = ctypes.create_string_buffer(size); info = _FILE_RENAME_INFORMATION.from_buffer(raw)
    info.u.ReplaceIfExists = 1 if replace_if_exists else 0; info.RootDirectory = wintypes.HANDLE(parent_handle); info.FileNameLength = len(encoded); ctypes.memmove(ctypes.addressof(raw) + offset, encoded, len(encoded)); iosb = _IO_STATUS_BLOCK()
    status = int(_NtSetInformationFile(wintypes.HANDLE(file_handle), ctypes.byref(iosb), ctypes.byref(raw), size, _FILE_RENAME_INFORMATION_CLASS))
    if status < 0:
...
def _nt_rename_relative_no_clobber(file_handle: int, parent_handle: int, leaf: str) -> None:
    _nt_rename_relative(file_handle, parent_handle, leaf, replace_if_exists=False)


def _nt_rename_relative_replace(file_handle: int, parent_handle: int, leaf: str) -> None:
    _nt_rename_relative(file_handle, parent_handle, leaf, replace_if_exists=True)


def _nt_mark_delete(file_handle: int) -> None:
    disposition = _FILE_DISPOSITION_INFORMATION(1); iosb = _IO_STATUS_BLOCK(); _NtSetInformationFile(wintypes.HANDLE(file_handle), ctypes.byref(iosb), ctypes.byref(disposition), ctypes.sizeof(disposition), _FILE_DISPOSITION_INFORMATION_CLASS)


class WindowsWorkspaceBackend:
    def __init__(self, workspace_root: str | os.PathLike[str]) -> None:
        raw = os.path.abspath(os.fspath(workspace_root)); handle = _CreateFileW(raw, _FILE_LIST_DIRECTORY | _FILE_READ_ATTRIBUTES | _SYNCHRONIZE, _FILE_SHARE_READ | _FILE_SHARE_WRITE, None, _OPEN_EXISTING, _FILE_FLAG_BACKUP_SEMANTICS | _FILE_FLAG_OPEN_REPARSE_POINT, None)
        if handle == _INVALID_HANDLE_VALUE: raise WorkspaceBoundaryError("workspace root could not be opened no-follow")
    ...
    def prepare_create(self, relative_path: str) -> "WindowsPreparedCreate":
        ...
        try:
            for component in parts[:-1]:
                child = _nt_open_relative(parent, component, desired_access=_FILE_LIST_DIRECTORY | _FILE_READ_ATTRIBUTES | _SYNCHRONIZE, share_access=_FILE_SHARE_READ | _FILE_SHARE_WRITE, disposition=_FILE_OPEN, options=_FILE_DIRECTORY_FILE | _FILE_OPEN_REPARSE_POINT | _FILE_SYNCHRONOUS_IO_NONALERT)
    ...
    def current_parent_identity(self, parent_parts: tuple[str, ...]) -> str:
        parent = self._root_handle; chain: list[int] = []
        try:
    ...
    def _revalidate_root_path(self) -> None:
        handle = _CreateFileW(self.canonical_root, _FILE_READ_ATTRIBUTES | _SYNCHRONIZE, _FILE_SHARE_READ | _FILE_SHARE_WRITE, None, _OPEN_EXISTING, _FILE_FLAG_BACKUP_SEMANTICS | _FILE_FLAG_OPEN_REPARSE_POINT, None)
        if handle == _INVALID_HANDLE_VALUE: raise WorkspaceBoundaryError("workspace root path is no longer available")
        try:
        ...
        finally: _win_close(int(handle))


class WindowsPreparedCreate:
    def __init__(self, backend: WindowsWorkspaceBackend, chain: list[int], parent_handle: int, parent_parts: tuple[str, ...], leaf: str) -> None:
        self._backend = backend; self._chain = chain; self._parent_handle = parent_handle; self._parent_parts = parent_parts; self._leaf = leaf; self._closed = False; self.parent_identity = _win_file_identity(_win_info(parent_handle)); self.revalidate_absent()
    ...
    def revalidate_absent(self) -> None:
        ...
        if self._backend.current_parent_identity(self._parent_parts) != self.parent_identity: raise WorkspaceBoundaryError("Windows parent path identity changed")
        if _win_file_identity(_win_info(self._parent_handle)) != self.parent_identity: raise WorkspaceBoundaryError("pinned Windows parent identity changed")
        target: int | None = None
        try:
            target = _nt_open_relative(self._parent_handle, self._leaf, desired_access=_FILE_READ_ATTRIBUTES | _SYNCHRONIZE, share_access=_FILE_SHARE_READ | _FILE_SHARE_WRITE, disposition=_FILE_OPEN, options=_FILE_NON_DIRECTORY_FILE | _FILE_OPEN_REPARSE_POINT | _FILE_SYNCHRONOUS_IO_NONALERT)
        except _NtOpenError as exc:
            if exc.status in {_STATUS_OBJECT_NAME_NOT_FOUND, _STATUS_OBJECT_PATH_NOT_FOUND}: return
            raise
        else:
            info = _win_info(target)
            if info.dwFileAttributes & _FILE_ATTRIBUTE_REPARSE_POINT: raise WorkspaceBoundaryError("target is a reparse point")
        ...
        try:
            ...
            if content:
                ...
                while offset < len(content):
                    ...
                    if not _WriteFile(wintypes.HANDLE(temp), pointer, chunk, ctypes.byref(written), None): raise _win_error("WriteFile failed")
            ...
            pre_publish_check(); _nt_rename_relative_no_clobber(temp, self._parent_handle, self._leaf); published = True
            final_info = _win_info(temp)
            if final_info.dwFileAttributes & (_FILE_ATTRIBUTE_REPARSE_POINT | _FILE_ATTRIBUTE_DIRECTORY): raise WorkspaceMutationError("published Windows target is not a regular file")
            return _win_file_identity(final_info)
        finally:
            if temp is not None:
                if not published:
