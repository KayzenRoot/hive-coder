...
_NTSTATUS = ctypes.c_long; _ULONG_PTR = ctypes.c_size_t
...
_STATUS_OBJECT_NAME_NOT_FOUND=0xC0000034; _STATUS_OBJECT_PATH_NOT_FOUND=0xC000003A; _STATUS_OBJECT_NAME_COLLISION=0xC0000035
_FILE_RENAME_INFORMATION_CLASS=10; _FILE_DISPOSITION_INFORMATION_CLASS=13
...
def _rename_buffer(parent_handle:int,leaf:str,flags:int):
...
def _nt_rename_relative_replace(file_handle:int,parent_handle:int,leaf:str)->None:
    """Atomic name replacement using Windows POSIX rename semantics.

    FILE_RENAME_POSIX_SEMANTICS allows replacement even when the old destination
    has open handles that share delete. It is still name-based publication, not
    an expected-file-id CAS predicate; CR-001's residual external race remains.
    """
    raw,size=_rename_buffer(parent_handle,leaf,_FILE_RENAME_REPLACE_IF_EXISTS|_FILE_RENAME_POSIX_SEMANTICS); iosb=_IO_STATUS_BLOCK(); status=int(_NtSetInformationFile(wintypes.HANDLE(file_handle),ctypes.byref(iosb),ctypes.byref(raw),size,_FILE_RENAME_INFO_EX_CLASS))
    if status<0: raise WorkspaceMutationError(f"NtSetInformationFile FileRenameInfoEx failed (ntstatus=0x{status&0xFFFFFFFF:08x})")
...
class WindowsPreparedCreate:
    ...
    def publish(self,content:bytes,pre_publish_check:Callable[[],None])->str:
        if self._closed:raise WorkspaceBoundaryError("prepared Windows target is closed")
        temp_name=f"{_TEMP_PREFIX}{secrets.token_hex(16)}.tmp"; temp=None; published=False
        try:
