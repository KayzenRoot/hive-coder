...
class ShadowBenchTests(unittest.TestCase):
    ...
    def test_tampered_hidden_case_fails_verification(self):
        case = self.factory.generate(
            self.snapshot, self.truth, BenchmarkDimension.SECURITY,
            epoch_label="epoch-001",
        )[0]
        forged = replace(case, mutation_kind="authority-confusion" if case.mutation_kind != "authority-confusion" else "trust-boundary")
        self.assertFalse(self.factory.verify_case(forged, self.truth))
...
class RecertificationTests(ExpertAgentFixture):
    ...
    def evidence(self, profile, *, observed_epoch=100, snapshot_digest=None):
        report = self.passing_report(profile)
        self.assertTrue(report.passed)
        return CertificationEvidence.from_report(
            "cert.one", report, snapshot_digest or snapshot().fingerprint(), observed_epoch,
            ("hive-fresh", "terminal-lab"),
        )
    ...
    def test_competence_half_life_moves_current_to_due_to_expired(self):
        ...
        current = clock.assess(
            ...
            current_epoch=110, evidence=(evidence,),
        )
        due = clock.assess(
            profile, standard, repository_snapshot_digest=snapshot().fingerprint(),
            current_epoch=140, evidence=(evidence,),
        )
        expired = clock.assess(
            profile, standard, repository_snapshot_digest=snapshot().fingerprint(),
            current_epoch=191, evidence=(evidence,),
        )
    ...
    def test_repository_snapshot_change_expires_previous_certification(self):
        ...
        report = RecertificationClock(self.profile_authority).assess(
            profile, self.standard(AgentRole.SECURITY),
            repository_snapshot_digest=sha("new-snapshot"), current_epoch=100,
            evidence=(evidence,),
        )
    ...
    def test_execution_stack_change_cannot_reuse_old_certification(self):
        ...
        report = RecertificationClock(self.profile_authority).assess(
            ...
            repository_snapshot_digest=snapshot().fingerprint(), current_epoch=100,
            evidence=(evidence,),
        )
        self.assertIs(report.status, RecertificationStatus.EXPIRED)
    ...
    def test_non_distinguished_standard_cannot_be_used_for_production_recertification(self):
        profile = self.profile(AgentRole.BACKEND, "backend-a")
        standard = replace(self.standard(AgentRole.BACKEND), level=self.standard(AgentRole.BACKEND).level.QUALIFIED,
                           min_samples_per_dimension=10, min_lower_bound=0.50, min_independent_suites=1)
        with self.assertRaises(ValueError):
            RecertificationClock(self.profile_authority).assess(
                profile, standard, repository_snapshot_digest=snapshot().fingerprint(),
                current_epoch=100, evidence=(),
            )

    def test_negative_outcome_can_force_due_but_positive_outcome_cannot_create_certification(self):
        profile = self.profile(AgentRole.BACKEND, "backend-a")
        evidence = self.evidence(profile)
        authority = OutcomeAuthority(b"outcome-authority-key-at-least-32-bytes!")
        ledger = OutcomeEchoLedger(authority)
        record = authority.seal(OutcomeRecord(
            "outcome.one", profile.fingerprint(), sha("plan"), snapshot().fingerprint(),
            105, True, 1, 0, sha("evidence"),
        ))
        ledger.add(record)
        signal = ledger.signal(profile.fingerprint(), current_epoch=110)
        due = RecertificationClock(self.profile_authority).assess(
            profile, self.standard(AgentRole.BACKEND),
            repository_snapshot_digest=snapshot().fingerprint(), current_epoch=110,
            evidence=(evidence,), outcome=signal,
        )
        self.assertIs(due.status, RecertificationStatus.DUE)
        self.assertTrue(signal.advisory_only)

        no_evidence = RecertificationClock(self.profile_authority).assess(
            profile, self.standard(AgentRole.BACKEND),
            repository_snapshot_digest=snapshot().fingerprint(), current_epoch=110,
