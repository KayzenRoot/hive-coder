...
from hive_runtime.agent_tasks import (
    ActionKind, AgentTaskRuntime, CheckpointStore, NodeStatus, RecoveryDisposition,
    StepResult, TaskBudget, TaskNode, TaskPlan, TaskStatus,
)
...
class PlanTests(unittest.TestCase):
    ...
    def test_prompt_hash_changes_fingerprint_without_exposing_prompt(self):
        a=TaskPlan((TaskNode("n",ActionKind.MODEL_PROMPT,prompt="secret one"),)); b=TaskPlan((TaskNode("n",ActionKind.MODEL_PROMPT,prompt="secret two"),))
        self.assertNotEqual(a.fingerprint(),b.fingerprint()); self.assertNotIn("secret",a.fingerprint())
...
class RuntimeTests(unittest.TestCase):
    ...
    def test_model_router_requirement_fails_closed(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(snap.status,TaskStatus.FAILED); self.assertEqual(dict(snap.node_status)["n"],NodeStatus.FAILED)
            self.assertEqual(snap.events[-1].code,"exception:LookupError")
    ...
    def test_global_execution_budget_pauses_and_can_resume_with_new_budget(self):
        with tempfile.TemporaryDirectory() as tmp:
            plan=TaskPlan((TaskNode("a",ActionKind.MODEL_PROMPT,prompt="a"),TaskNode("b",ActionKind.MODEL_PROMPT,depends_on=("a",),prompt="b")))
            rt,_=runtime_for(tmp,plan,budget=TaskBudget(max_executions=1,max_failures=1),model_router=router()); rt.create("t")
            snap=rt.run_until_blocked(); self.assertEqual(snap.status,TaskStatus.PAUSED); self.assertEqual(snap.executions,1)
            rt2,_=runtime_for(tmp,plan,budget=TaskBudget(max_executions=3,max_failures=1),model_router=router()); rt2.resume("t"); rt2.continue_task(); self.assertEqual(rt2.run_until_blocked().status,TaskStatus.SUCCEEDED)
    ...
    def test_snapshot_is_not_mutable_authority(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIsInstance(snap.node_status,tuple)

    def test_invalid_port_result_fails_closed(self):
        with tempfile.TemporaryDirectory() as tmp:
            class Bad(Ports):
                def prompt(self,*args): return {"success":True}
            plan=TaskPlan((TaskNode("n",ActionKind.MODEL_PROMPT,prompt="x"),)); rt,_=runtime_for(tmp,plan,Bad(),model_router=router()); rt.create("t")
            snap=rt.run_until_blocked(); self.assertEqual(snap.status,TaskStatus.FAILED); self.assertEqual(snap.events[-1].code,"invalid_port_result")

    def test_malicious_result_code_not_persisted(self):
        with tempfile.TemporaryDirectory() as tmp:
            ports=Ports(); ports.prompt_results=[StepResult(False,"TOKEN=super-secret value",False)]
            ...
            self.assertEqual(snap.events[-1].code,"invalid_result_code"); self.assertNotIn("super-secret",Path(tmp,"t.json").read_text())

    def test_exception_message_not_persisted(self):
        with tempfile.TemporaryDirectory() as tmp:
            class Boom(Ports):
                def prompt(self,*args): raise RuntimeError("provider-token-super-secret")
            ...
            raw=Path(tmp,"t.json").read_text(); self.assertNotIn("provider-token-super-secret",raw); self.assertIn("exception:RuntimeError",raw)

    def test_raw_prompt_is_not_persisted(self):
        with tempfile.TemporaryDirectory() as tmp:
            secret="never-persist-this-prompt-secret"; plan=TaskPlan((TaskNode("n",ActionKind.MODEL_PROMPT,prompt=secret),)); rt,_=runtime_for(tmp,plan,model_router=router()); rt.create("t")
            self.assertNotIn(secret,Path(tmp,"t.json").read_text())
...
class CheckpointTests(unittest.TestCase):
    ...
    def test_checkpoint_contains_no_authority_fields(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for forbidden in ("credential","permit","approval_token","capability_grant","skill_content","model_output"):
