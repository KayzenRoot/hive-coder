...
- `HCODER-WO-0026` (Issue `#85`, slice `HCODER-DIST-001C`) release trust substrate is a **promotion candidate** in Draft PR `#86`, which is OPEN / DRAFT / UNMERGED: the `hive-release-provenance-v1` contract, the offline validator `tools/desktop/release_provenance.py` and the `Protected Release` lane are IMPLEMENTED and were independently reviewed at `0a1b8375e5c950d321f898b6d455eb874583c9a4` with CRITICAL `0` / HIGH `0` / MEDIUM `0` / LOW `0`, so technical behavior is frozen for promotion prep. `DEC-030` is **APPROVED FOR PROMOTION CANDIDATE / NOT CANONICAL ON MAIN until `HCODER_CP_0026_EFFECTIVE`**, and its checkpoint delta `.engineering/checkpoint-deltas/HCODER-WO-0026.md` is **PROPOSED / REVIEW PENDING**. That approval is not promotion: the promotion-candidate head needs fresh exact-head gates and another independent review, then a governed merge and exact-`main` post-validation. No `HCODER-CP-0026` may be claimed from the candidate, and its credential-bearing stages remain structurally present but unexercised: no signing credential, no protected environment and no notarization credential exist on the host.
