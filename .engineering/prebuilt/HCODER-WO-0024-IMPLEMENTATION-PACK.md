...
**Prior reviewed-head facts:** historical only; the reviewed-head record and the correction narratives live in `.engineering/evidence/HCODER-WO-0024.md`, and mutable current review and gate state is external in PR #80 and Issue #30. This document does not mirror the current review index.  
**Canonical authority history:** `.engineering/context-locks/HCODER-WO-0024.md` — the append-only Context Lock delta sequence for this Work Order; no terminal delta number or range is mirrored here.  
...
- The declared profile is **toolchain-compatible bounded SemVer 2.0.0** with two project bounds: each core identifier within `0..9007199254740991` and the whole version string at most 128 characters. Both bounds are part of the law and are enforced identically by the product TypeScript parser and the Python drift gate.
- The core bound follows the **narrowest declared mirror consumer**. `Cargo.toml` and `package.json` are both exact mirrors; Cargo's Rust SemVer accepts core values up to `u64::MAX`, npm's `node-semver` rejects any core component above `Number.MAX_SAFE_INTEGER`, so the npm bound governs. A version this product accepts must be parseable by every mirror.
- Core identifiers are carried as exact decimal strings, bounded by exact string logic and compared by digit length then lexicographically. No `Number()` conversion and no platform-dependent integer coercion anywhere on the path.
- Numeric *prerelease* identifiers are deliberately not core-bounded: no declared consumer imposes a bound there, so they remain exact at any length within the overall string bound.
...
- A status snapshot claiming an authenticity-dependent state must carry a proof accepted under the same policy. Because no scheme is admitted, no such state can be entered or asserted as a current status in this slice — while a refused attempt toward `ready` remains recordable with the bounded refusal outcomes.
...
## Unit tests required before promotion
Contract tests must cover: SemVer acceptance and rejection under the toolchain-compatible bounded profile; the core bound at `MAX_SAFE-1`, `MAX_SAFE`, `MAX_SAFE+1`, `u64::MAX`, `u64::MAX+1` and the former oversized-core vectors; exact ordering of accepted core values with no precision collapse; exact numeric comparison for prerelease identifiers above `2^53`; cross-language parity vectors consumed by both the TypeScript and Python suites (core boundaries, large prerelease values, malformed forms, trailing LF/CR/CRLF and Unicode line separators, non-ASCII and Unicode-digit content, the 128-character boundary built from legal non-core content); canonical/mirror drift, missing and malformed mirrors; channel vocabulary and unknown-channel refusal; version-shape/channel matching derived from the single canonical parser; same-channel upgrade eligibility; downgrade, identical-version and prerelease-mismatch refusal; cross-channel refusal with governed-decision reporting; the full transition table including illegal transitions; proof-gated refusal on every edge entering an authenticity-dependent state; exhaustive unreachability of `ready`, `installing` and `success` as transition destinations; malformed proof refusal; bounded and credential-shaped error-detail refusal; status closure (unknown keys at every level, per-event validation, required keys, candidate and error invariants, canonical reconstruction); refusal of direct authenticity-dependent snapshots; the persisted-event matrix (unreachable source states rejected for every destination and reason, `verifying -> ready` accepted only with the bounded refusal outcomes, ordinary edges accepted only with `legal_transition`, `illegal_transition`/`unknown_state` refused on persisted events); plain own-data record validation (inherited fields, class instances, accessor-backed records, non-enumerable and symbol keys, sparse and accessor-backed arrays) with a proven zero getter-invocation count; version/channel identity at every boundary; inert-boundary member absence; and absence of any network/process/update side-effect surface.
