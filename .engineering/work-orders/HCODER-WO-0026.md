...
6. **Least privilege by construction.** Repository-wide workflow permissions stay read-only; `id-token`, `attestations`, `artifact-metadata` and `contents: write` are granted only to the single job that needs each, and only on the promotion path. No `pull_request_target` or equivalent, so release credentials are never exposed to pull-request code. `id-token: write` today reaches the attestation jobs and no other: the credential-bearing signing jobs hold neither `id-token` nor any expression able to resolve a slot, and the grant a real signing executor will need is deferred to the governed slice that implements signing rather than smuggled in with provisioning.
...
- An attestation is evidence about this release only if its subject set is exactly this release's package identities, derived from the validated inventory document and never from a filesystem walk, a glob or the manifest file standing in for the packages it describes. Byte-for-byte packages are attested by file digest; a directory bundle such as the macOS `.app` is attested by its sorted-tree digest, which is the identity the inventory already records for it.
...
- Attested subjects are the CP-0025 package identities, and each build lane proves after attesting that the signed statement's subject set equals the inventory's package set exactly: no missing subject, no extra subject, no neighbouring-build subject.
- Each signing stage resolves nothing: it holds no secret binding, no presence probe and no signing command, so its reported `VALUES_READ=NO` stays true after someone provisions a credential instead of becoming false at exactly the moment it matters.
